...
![Validation-PPL trajectory for the two d=384, L=16 aniso-Gaussian + Fock-reg full runs, gamma_train=0.10 (green, continues to step 11,500) versus gamma_train=0.30 (red, killed around step 8,039), log-scaled y-axis, with vertical dashed lines marking each run's watchdog reload steps and a horizontal reference line at gamma=0.30's best-ever val_ppl of 215.09](images/gamma_d384_ppl_comparison.png)

*Figure: as of this writing the γ=0.30 run has not yet been extended to a matched step-11,000 endpoint — see the note at the end of this subsection. Regeneration script: `paper_v5/figures/_make_gamma_d384_ppl_comparison.py`.*
...
**Decision to extend the $\gamma=0.300$ run to a matched endpoint before killing it.** The comparison above already uses a mismatched horizon — $\gamma=0.100$'s numbers run through step 11,500 while $\gamma=0.300$ stops at ~8,039 — which leaves an obvious objection available to a skeptical reader: is $\gamma=0.100$'s lead simply because it has had ${\sim}3{,}500$ more steps to compound its (already-established, §11.7) tighter fit to the training distribution? The decisive numbers for the thesis (2 reloads vs. 0, a stalled best-so-far vs. a still-improving one) are already visible by step 8,000 and are unlikely to reverse with more compute, so the marginal *evidentiary* value of extending is modest. But the marginal *cost* is small relative to the run's own history (each 500-step eval window above took a few minutes of wall time on this hardware, so ${\sim}3{,}000$ more steps is well under an hour), and a matched step-11,000 (or 11,500) endpoint removes the horizon-mismatch objection entirely and gives a like-for-like final row for \cref{fig:gamma-d384-ppl} and any comparison table built from it. **Recommendation: let the $\gamma=0.300$ run continue to step ${\approx}11{,}000$–$11{,}500$ (matching $\gamma=0.100$'s last logged checkpoints exactly), then kill it.** Two caveats attached to that recommendation: (a) this run has already disconnected once from this Colab session (§12.5's opening paragraph), so checkpoint frequency should be confirmed before walking away from it unattended for the additional ${\sim}3{,}000$ steps; (b) if a third watchdog reload occurs before step 11,000, that is itself useful confirmatory evidence (a rising reload rate under continued training) and is not a reason to extend further — the stopping rule is the step count, not "wait until it stabilizes."
