...
4. **The non-monotonic spike at $\gamma=0.30$.** The training gamma of the flagship d=384 run (e5c_plgate) has the highest $\bar{R}$ in the entire sweep (2.115), sitting *above both* its neighbors ($\gamma=0.25$: 1.418, $\gamma=0.40$: 1.636). The per-layer heatmap (Fig. 7) shows this is driven by localised departures in the middle layers (6–10) rather than uniform degradation — a structural resonance at this specific damping value, not a general monotonic trend.

5. **$\bar{R} \gg 1$ does not mean "not a geodesic."** The residual tests compliance with the damped geodesic equation *at the nominal training $\gamma$*. A large $\bar{R}(\gamma_{\text{train}})$ means the nominal hyperparameter is a poor match for the trajectory's intrinsic damping — it does *not* mean the trajectory has left the Riemannian manifold or stopped following a damped geodesic altogether. The $\gamma_{\text{geo}}$ column recovers the damping the trajectory actually exhibits: for $\gamma_{\text{train}}=0.30$, $\gamma_{\text{geo}} \approx 0.995$ (near-critically-damped). If the residual were re-evaluated at $\gamma_{\text{eval}}=\gamma_{\text{geo}}$, it would drop back toward $\bar{R} \approx 1$. The model follows a damped geodesic at its *own intrinsic damping* regardless of the training hyperparameter.

   This distinction is important for downstream capabilities. Both **energy-dissipation hallucination detection** (§18d Experiment G2) and **native chain-of-thought via the Fock register lifecycle** (§18d Experiment G3) require only that the model's dynamics satisfy *some* damped geodesic equation with a well-defined Jacobi metric — they do not require the nominal $\gamma$ to match the intrinsic one. The metric is guaranteed to be valid (it is a consequence of having an exact potential $V_\theta$), so the full geometric toolkit — directed geodesic distance, energy-dissipation anomalies, register-waypoint reasoning — remains available at every $\gamma$, including the $\gamma=0.30$ operating point of the flagship run.
