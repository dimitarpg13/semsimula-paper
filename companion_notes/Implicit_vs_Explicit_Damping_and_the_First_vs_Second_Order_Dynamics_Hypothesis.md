...
### 3.1 A per-layer retention of ~0.51 is not close to the asymptotic reduction

The first-order gradient-flow limit (`Replacing_The_Conservative_Mechanism_of_SPLM_with_First_Order.md`, §3.1) is the regime $\gamma \gg \omega_0$ (friction much larger than the system's natural frequency), where the velocity term becomes negligible *at every step* and the equation of motion collapses to $\dot h = -\tilde\beta \nabla V_\theta$ with $\tilde\beta = \beta/\gamma$. That is a statement about $r_{\text{layer}} \to 0$ — velocity forgotten almost entirely, layer to layer. The empirical $r_{\text{layer}}(\gamma_{\text{geo}}) \approx 0.51$ says the *opposite*: roughly half the previous displacement is still carried forward at every single layer. That is a moderate-damping regime, not deep overdamping. It takes many layers of compounding (not a single layer's ~50% retention) for the *cumulative* effect over a full stack to look small — and the local, per-layer mechanism (blend previous velocity with newly computed force, then step) is structurally different from a first-order recursion at each individual layer, even when the long-range cumulative retention is tiny.
...
**Conclusion of this section:** $\gamma_{\text{geo}} \approx 0.95$ is real, reproducible, and architecturally important — but it characterizes the *inference-time trajectory of an already-trained second-order model*. It is silent on what would happen if the velocity state were removed at initialization and the network were trained from scratch as a first-order system. That is a separate, structurally different question, addressed next.
...
- **How the $\gamma_{\text{geo}}$ finding relates:** it is an independent, closed-form corroboration of the same qualitative picture, via a completely different methodology (Jacobi-metric geodesic fitting on Fock-PARFLM rather than Markov-order regression on GPT-2/Pythia), and it adds a quantitative refinement the Markov-order test cannot provide: the effective damping is not "first-order," it is $\gamma_{\text{geo}} \approx 0.95$, corresponding to $r_{\text{layer}} \approx 0.51$ — a specific, moderate, non-trivial value, not the $r_{\text{layer}} \to 0$ that true first-order dynamics would require. Both pieces of evidence support "not rejected as approximately first-order at this resolution," but neither supports "is exactly first-order."
...
- **Two damping channels exist and are separable.** The explicit $\gamma_{\text{train}}$ dial and the implicit damping contributed by LayerNorm and potential curvature are different quantities; the latter dominates, converging to $\gamma_{\text{geo}}\approx0.9$–$0.98$ ($r_{\text{layer}}\approx0.51$) across every scale, corpus, and $V_\theta$ family tested to date.
- **This does not imply a first-order reduction would match second-order performance.** $r_{\text{layer}}\approx0.51$ is a moderate-damping regime, not the asymptotic $r_{\text{layer}}\to0$ that licenses the first-order reduction; the reduction also collapses two independently-tunable quantities into one; and the existing sweeps confound higher $\gamma$ with smaller effective step size.
