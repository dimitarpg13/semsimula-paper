...
- **Training-phase and inference-phase are separate claims with separate, differently-resourced evidence.** The inference-phase claim (first-order not rejected at the trained fixed point) is **settled** (Outcome C, executed 2026-04-27, on GPT-2/Pythia). The training-phase claim (inertia adds genuine value during optimization) has been **executed and confirmed for the original MLP-$V_\theta$ SPLM family on TinyShakespeare** (`SPLM-1_ablation_pre-registered_protocol.md`; the initial $\gamma=0.30$ leak-free retrain was a non-detection, but a follow-up $\gamma$-sweep located the PPL-optimal damping at $\gamma^\ast\in[0.10,0.15]$, where an $S=5$ confirmation sweep clears every pre-registered criterion for $H_1$, $\overline\Delta=+5.09$ to $+7.03$ PPL — §4.2), but **has no completed analogue yet for the anisotropic Gaussian $V_\theta$ + Fock-reg family** that the current scale-up work actually uses: the Fock-G1 protocol (§6) is in progress on TinyStories and not yet run on OpenWebText.
...
- **§6 proposes Fock-G1**, a fixed-step-size, matched-compute first-order ablation of the aniso-Gaussian architecture, anchored at the $d=384$ sweep winner ($\gamma^\ast=0.10$) with an optional $d=768$ or $d=1024$ secondary arm — the latter now the better-motivated choice, given its wider (~10–11%) boundary-optimum margin — as the concrete next experiment to close this gap for the architecture family this line of work is actually built on.
