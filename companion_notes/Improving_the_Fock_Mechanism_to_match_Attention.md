...
### 10.4 The Transfer Mechanism: How a Token Deposits Content into a Register

The reverse-channel force $Q_i$ of §10.1 and the creation readout of §9.1 are normally written as algebraic assignments — a register is simply set to a weighted sum of token values:

$$r_k = \sum_j \alpha_{kj} v_j.$$

That phrasing hides the mechanics and obscures **why** the exchange is non-conservative. To make the dynamics explicit it is more faithful to treat each register $r_k$ not as a slot that is overwritten, but as a genuine **particle coordinate** living in the same semantic space $\Sigma$ as the tokens. The full system then carries the joint coordinates

$$(h_1, \ldots, h_T, r_1, \ldots, r_M) \in \mathbb{R}^{(T+M) \times d},$$

and "depositing content" must be expressed as a **force acting on the register coordinate**, generated by the token particles. This is the missing link between the Q/K/V attention readout and the inter-particle dynamics of the conservative base model.

### 10.5 The Creation Vertex as an Alignment-Gated Spring Bundle

Give each token $j$ a **key** $k_j$ (its label in coupling space) and a **content charge** $v_j$, and give register $k$ a **query** $q_k$. The creation vertex is then an interaction energy that ties the register to the tokens through springs whose **stiffness is the query-key alignment**:

$$V_{\text{create}}(h, r) = \sum_k \frac{1}{2} \sum_j \alpha_{kj}(h) \lVert r_k - v_j \rVert^2, \qquad \alpha_{kj} = \mathrm{softmax}_j\left( \frac{q_k \cdot k_j}{\tau} \right).$$

Each token contributes one Hookean spring to register $k$: a spring of stiffness $\alpha_{kj}$ whose natural rest position is the content $v_j$. The force on the register is the gradient of this energy:

$$F_{r_k} = -\nabla_{r_k} V_{\text{create}} = \underbrace{\sum_j \alpha_{kj} v_j}_{\text{weighted content}} - r_k.$$
...
### 10.6 Depositing Is Overdamped Relaxation in the Spring Field

A register is a light, fast auxiliary particle: in the overdamped limit it relaxes to the force-balance point $F_{r_k} = 0$,

$$r_k \longrightarrow \sum_j \alpha_{kj} v_j.$$

This recovers **exactly** the algebraic creation readout of §9.1 — but now as the **fixed point of a spring relaxation**, not as an assignment. It is the same overdamped reduction that connects the second-order token flow to first-order JEPA-style updates as $\gamma \to \infty$ (§10.1 and Paper v4); the registers simply sit at that fast limit. In the QFT language of §5, the register is a virtual mediator that is created (the spring bundle forms), absorbs content (relaxation), and is later annihilated (decay).

### 10.7 Where the Conservative Symmetry Actually Breaks

Here is the subtlety the particle picture makes precise. If the spring energy $V_{\text{create}}$ were the *entire* interaction, the joint $(h, r)$ system would be **conservative** — a single potential — and Newton's third law would hold: each token would feel the equal-and-opposite spring force $-\nabla_h V_{\text{create}}$. Integrating the registers out of one conservative joint potential leaves an effective token-token potential that is **still symmetric**, so the cross-Jacobian reciprocity of §4.1 would be restored and attention's asymmetry (P1) would be lost. **A single shared force field cannot break the symmetry.**

The symmetry breaks because the **reverse channel is a separately parameterised force field**, not the back-reaction of $V_{\text{create}}$:

$$F_{h_i}^{\text{reverse}} = \sum_k \beta_{ik} (\tilde{v}_k - h_i), \qquad \beta_{ik} = \mathrm{softmax}_k\left( \frac{Q_i \cdot K_k}{\sqrt{d}} \right),$$

with **independent** projections: the token query $Q_i$, the register keys $K_k$, and the register values come from different weight matrices than the $q_k, k_j, v_j$ of the creation vertex. Because the forward stiffness $\alpha_{kj}$ and the backward stiffness $\beta_{ik}$ are produced by two different rules, there is **no single elastic potential** whose gradient yields both vertices. Newton's third law fails at the vertex level:

$$\frac{\partial F_{r_k}}{\partial h_j} \neq \frac{\partial F_{h_j}}{\partial r_k}.$$

When the registers are integrated out, the effective token-to-token influence is a product of two *different* asymmetric stiffness matrices,

$$A_{ij} \sim \sum_k \beta_{ik} \alpha_{kj}, \qquad A_{ij} \neq A_{ji},$$

which is asymmetric and is not the gradient of any scalar — exactly the non-conservative, reciprocity-violating coupling that the Conservative Obstruction Theorem (§4, §15) forbids on the bare token subsystem. Two further ingredients complete the picture. First, the influence is **routed through a small shared pool** of $M$ mediators rather than through $O(T^2)$ direct bonds, which is what keeps the inference state $O(1)$ in sequence length. Second, the registers **decay** (the destruction gate, salience $\sigma_k \to \lambda \sigma_k$ with $\lambda \lt 1$), so the spring bundles are **transient**: set up, used to carry one asymmetric pulse of content, then dissipated — the hallmark of a *virtual* exchange quantum rather than a permanent conservative bond.

Crucially, all of this symmetry breaking is **quarantined in the register sector**. The token subsystem retains its conservative second-order flow
...
## 19. Resolving the Conservative–Attention Gap

The register diversity/entropy eval pass (§18.5) has been completed, and the FockAttentionPARFLM experiment (§5.1 literal Feynman diagram) has produced initial results. This section consolidates the findings and charts the resolution path.
...
**Step 3 — Sparse top-k creation (if M genuinely needs to scale)**
...
**Step 4 — Iterative register refinement (Perceiver-style)**

R=2–3 rounds of token→register→register→token attention at O(R·M·T + M²) cost for M ≪ T.

### 19.6 Step 1 Experimental Plan

The Step 1 fixes are implemented via new config fields in `FockMultiXiPARFConfig` (`per_register_tau`, `per_register_keys`, `ortho_register_init`) and the `QKVCreationGate_v21` class, backward-compatible with existing v2 checkpoints.

The experiment (`colab_fock_v21_routing_fix.ipynb`) compares five arms against the v2 baseline (12.00 PPL):
...
All arms use τ_create_init = √d_k = 8.0 (for v2.1 arms) instead of the original 0.1 (which caused the temperature collapse). The most informative arm to run first is `v21_tau_only` — if per-register temperature alone fixes the collapse, temperature was the sole bottleneck.
