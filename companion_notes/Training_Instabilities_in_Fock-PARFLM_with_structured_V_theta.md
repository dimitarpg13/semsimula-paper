...
## 24–28. CfC/BAOAB Propagator and Stiffness Mitigations — moved

The BAOAB + CfC propagator analysis (former §24), the late-training-spike and
damping-hypothesis cascade analysis (former §25-§26), the empirical
depth-code-growth findings (former §27), and the proposed plus forward-looking
stiffness mitigations (former §28 and the new §29-§30) now live in a dedicated
companion note,
[CfC_BAOAB_Integrator_and_Mitigations.md](CfC_BAOAB_Integrator_and_Mitigations.md),
to keep this document manageable. Section numbers are preserved there, so all
`§24`-`§28` cross-references throughout this note still resolve.
...
*Last updated: 24 August 2026. Sections 24-28 (BAOAB+CfC propagator,
late-training/damping cascade analysis, empirical depth-code growth, and the
$B_k$ clamp mitigation), together with the new forward-looking §29-§30, were
moved to the companion note `CfC_BAOAB_Integrator_and_Mitigations.md` for
maintainability; section numbering is preserved there. See that note for all
CfC/BAOAB-related discussion.*
