...
**Position:** fifth candidate construction for the hybrid programme of the v3 paper section 17.3 (Q9), and a successor to [`Scalar_Potential_based_Helmholtz_Architecture_v2.md`](./Scalar_Potential_based_Helmholtz_Architecture_v2.md). We label this **Q9(e): the attention-free Helmholtz hybrid**, also called **Solenoidal-Pair Hybrid SPLM (SP-HSPLM)** or, more colourfully, **Maxwell-PARFLM**.
**Audience:** internal — collaborators, reviewers, and the companion-notes track of the paper.
**Companion docs:**

- [`Scalar_Potential_based_Helmholtz_Architecture_v2.md`](./Scalar_Potential_based_Helmholtz_Architecture_v2.md) — Q9(d), the attention-using Helmholtz hybrid.
...
- [`parf/On_Gumbel_softmax_sparsity_applied_to_V_phi.md`](./parf/On_Gumbel_softmax_sparsity_applied_to_V_phi.md) — the routing primitive reused here.
...
This is autonomous and conservative because the force is the gradient of a sum of scalars; PARFLM lives here. The "shared across $\ell$" qualifier is the **SPLM autonomous-in-$\ell$ commitment**: at every layer of the integrator the same parametric family produces the force, and depth is exclusively the discretisation grid of a single ODE. This is the canonical SPLM commitment of paper §15 (in contrast to attention, where each layer instantiates an independent parameter tensor $\theta_\ell$). The architecture v3 specification preserves the commitment for all four learnable submodules ($V_\theta, V_\phi, J_\phi, \Omega, \alpha_\phi$). The Stage 2 Mechanism-1 extension cells (q9e_h/i/j/k in [`SP_HSPLM_Stage_2_pre-registered_protocol.md`](./SP_HSPLM_Stage_2_pre-registered_protocol.md) §3.1) lift the commitment one submodule at a time and provide the empirical test of whether the commitment is the load-bearing reason for the v3 paper §15.5 cluster ceiling.
...
The non-mass / non-damping submodules $V_\theta, V_\phi, J_\phi, \Omega, \alpha_\phi$ are likewise **shared across all $\ell$ of the relevant block type** in the v3 specification (the SPLM autonomous-in-$\ell$ commitment of §3.2). This makes the architecture a single shared ODE force law evaluated at $L$ integration steps, the canonical SPLM reading. It is **not** how attention works: paper Appendix A (Eq. A.130) shows that attention's Class-F expressivity comes from the *joint* application of two mechanisms — **Mechanism 2**, prefix conditioning of the force argument through $\xi_t$ (already present in $V_\phi, J_\phi$ via their dependence on $h_s$ for $s \lt t$), and **Mechanism 1**, a per-layer-indexed parametric family $\theta_\ell$ for the force law. The v3 specification deliberately omits Mechanism 1, retaining only Mechanism 2. The Stage 2 Mechanism-1 extension cells (q9e_h/i/j/k) are the test of whether re-introducing Mechanism 1, via `nn.ModuleList(L_C)` per-layer $J_\phi^{(\ell)}$ (and analogous for $V_\phi$, $\alpha_\phi$, $\Omega$), closes the residual gap to attention without instantiating any attention layer.
...
|---|---|---|
| Causal probe ([`causal_probe_parf.py`](../notebooks/conservative_arch/parf/causal_probe_parf.py)) | $0.0$ leak floor | The .detach() points are preserved across the C-block forward |
...
| Per-token scalar potential V_theta(xi, h) | Conservative single-particle force; shared across all S-blocks (SPLM autonomous-in-ell commitment, §3.2) | small MLP, ~2k params | O(B · T · d · H_theta) |
| Conservative pair scalar V_phi(h_t, h_s) | Conservative routing; reused from PARFLM, shared across all S-blocks (lifted to per-S-block in q9e_i / q9e_k, Mechanism-1 cells) | structural or MLP, ~4-28k params | O(B · T · k · d) at k-sparsity |
| Gumbel score head alpha_phi(h_t, h_s) | Pair affinity for both V_phi and J_phi routing; shared across all layers (lifted to per-layer in q9e_j / q9e_k) | small MLP on (h_t, h_s, h_t - h_s), ~8-16k params | O(B · T² · d · H_s) |
| Solenoidal pair kernel J_phi(Δh) | Skew-matrix-valued pair force; new component, shared across all C-blocks (lifted to per-C-block in q9e_h / q9e_k) | low-rank, J_+ = U V^T, U,V in R^(d × r) | O(B · T · k · d · r) at sparsity |
| Per-token gyroscopic kernel Ω(h_t) | Optional skew velocity coupling; per-token; shared across C-blocks when active (per-C-block hook reserved, not in the Stage 2 cell ladder) | low-rank, similar to J_phi | O(B · T · d · r) |
...
Following the structure of [`On_Training_the_PARF_Force.md`](./parf/On_Training_the_PARF_Force.md):
...
**Pre-registered protocol:** [`SP_HSPLM_Stage_1_pre-registered_protocol.md`](./SP_HSPLM_Stage_1_pre-registered_protocol.md). Implementation lives at `notebooks/conservative_arch/non_conservative/`.
...
**Pre-registered protocol:** [`SP_HSPLM_Stage_2_pre-registered_protocol.md`](./SP_HSPLM_Stage_2_pre-registered_protocol.md). Implementation will live at `notebooks/conservative_arch/sphsplm/`.
...
**Stage 2 extension — Mechanism-1 cells (q9e_h / q9e_i / q9e_j / q9e_k).** Added 16 May 2026 after the q9e_a/b/c readings (PPL $\approx 27.6$ / $28.1$ / $27.7$) confirmed that the Mechanism-2 capacity sweeps in $k$ and $r$ alone do not move SP-HSPLM off the 26-28 PPL cluster ceiling shared with em\_ln, PARFLM, and SparsePARFLM. Paper Appendix A (Eq. A.130) decomposes attention's expressivity into Mechanism 2 (prefix conditioning, already present in q9e_a/b/c/d through $V_\phi(h_t, h_s)$ and $J_\phi(h_t - h_s)$ via their $s \lt t$ dependence) and Mechanism 1 (per-layer-indexed parametric family $\theta_\ell$). The v3 specification (§3.2, §3.3, §4.1) deliberately omits Mechanism 1 in favour of the SPLM autonomous-in-$\ell$ commitment. The four Mechanism-1 cells lift the commitment one submodule at a time:

- **Q9e-H:** == Q9e-A but with per-layer $J_\phi^{(\ell)}$ (one independent low-rank skew kernel per C-block, via `nn.ModuleList(L_C)`). The most direct analogue of attention's per-layer $W_Q, W_K, W_V$ from inside the vector-field-theoretic framework.
- **Q9e-I:** == Q9e-A but with per-layer $V_\phi^{(\ell)}$ (one independent conservative pair scalar per S-block).
- **Q9e-J:** == Q9e-A but with per-layer $\alpha_\phi^{(\ell)}$ (one independent score head per layer; routing topology becomes layer-dependent).
- **Q9e-K:** == Q9e-A but with all three above per-layer (joint Mechanism-1). The cleanest empirical test of the paper Appendix A two-mechanism decomposition without instantiating any attention layer.

The H5 decision rule is stated in [`SP_HSPLM_Stage_2_pre-registered_protocol.md`](./SP_HSPLM_Stage_2_pre-registered_protocol.md) §2 H5 and §6.3 Outcomes ZETA / ETA. The Mechanism-1 cells are *not* parameter-matched against P10g (the per-layer overhead is several-fold larger than the Mechanism-2 overhead), so H5 is read relative to Q9e-A rather than to P10g. The implementation is in `notebooks/conservative_arch/sphsplm/model_sphsplm.py` via four `share_*_across_layers` config flags on `SPHSPLMConfig` (default `True` preserves §3.2's autonomous-in-$\ell$ commitment; the four Mechanism-1 cells flip one or more to `False`).

**Cost (Mechanism-1 extension):** 4 cells $\times$ 16k steps $\approx$ 4 GPU-days.

**Stage 2 extension — full Class-F cell (q9e_n).** Added 17 May 2026 after q9e_l (q9e_d + q9e_h, the Mechanism-1 × Mechanism-2 additivity cell) executed at 3 seeds with mean PPL 25.64 ± 1.03 (median 25.11). The 3-seed reading confirmed the H6 synergy at the median (the first SP-HSPLM cells below the P10g 26.42 ceiling) but also revealed that the in-class mechanisms tested so far cover only ≈ 1.3 of the ≈ 17 PPL **across-class** gap to MatchedGPT (≈ 8 PPL on the same scale). The diagnosis: q9e_a..q9e_m all implement a strict *subset* of Class F — the dominant scalar potential $V_\theta$ is shared across layers in every cell, even though Class F prescribes per-layer $\theta_\ell$ for $V_\theta$ exactly as for $J_\phi$ and $\Omega$. **Q9e-N lifts the V_θ sharing constraint** (plus per-layer V_φ and per-layer α_φ, the small Mechanism-1 axes not yet tested) and is the first true Class-F realisation. The implementation adds a fifth `share_v_theta_across_layers` flag to `SPHSPLMConfig`; per-layer V_θ is indexed by S-block position (V_θ is consumed only at S-blocks in the current SP-HSPLM design). At $d{=}256, v_{\mathrm{hidden}}{=}1024, v_{\mathrm{depth}}{=}3$ each V_θ copy is ≈ 2.6 M parameters, so $L_S = 4$ per-layer V_θ copies add ≈ 10 M parameters (≈ +70 % vs P10g). The H8 decision rule (protocol §2 H8, §6.3 Outcomes THETA / IOTA) is therefore stated relative to **q9e_l multi-seed median** rather than P10g, to isolate the V_θ-sharing contribution from the rest of the architecture. Three branches: THETA (PPL ≤ 12; attention is in Class F as Appendix A claims), IOTA-partial (PPL ∈ (12, 22]; Class F closes a substantial fraction of the gap, residual lives beyond Class F), IOTA-negative (PPL ≥ 22; Class F as currently realised is not attention's class, refutation of Appendix A's central reading).

**Cost (full Class-F extension):** q9e_n at 3 seeds $\approx$ 6 GPU-days (≈ 2 H100-h per seed at ≈ 26 M params). **Total Stage 2 cost** becomes 7 + 4 + 2 (q9e_l/m additivity-and-maximal) + 6 (q9e_n) = 19 GPU-days first cut, plus q9e_l-seed3 (1.3 H100-h) for the strict-ALPHA gating run. Still under half of the architecture-doc budget projection.
...
Stage 1 + Stage 2 + Stage 3 $\approx$ 16 GPU-days at H100/A100 in the original v3 specification; with the Stage 2 Mechanism-1 extension (q9e_h/i/j/k, +4 GPU-days), the q9e_l/m additivity-and-maximal cells (+2 GPU-days), and the q9e_n full-Class-F extension (+6 GPU-days at 3 seeds), the total rises to $\approx 28$ GPU-days. Still comparable to a single P10 sub-row of PARFLM, and includes the across-class Class-F test that directly addresses the residual ≈ 17 PPL gap to MatchedGPT revealed by the q9e_l 3-seed reading.
...
2. Add a per-token solenoidal cell to `notebooks/conservative_arch/parf/model_parf_sparse.py` — this is the **E4-fix** cell of Stage 1.
3. Write the C-block (`_c_block_step`) parallel to `_s_block_step` in [`notebooks/conservative_arch/helmholtz/model_helmholtz.py`](../notebooks/conservative_arch/helmholtz/model_helmholtz.py), reusing the SparsePARFLM score head and Gumbel mask.
...
- [`Scalar_Potential_based_Helmholtz_Architecture_v2.md`](./Scalar_Potential_based_Helmholtz_Architecture_v2.md) — Q9(d), the attention-using Helmholtz hybrid; the predecessor of this doc.
...
- [`parf/On_Gumbel_softmax_sparsity_applied_to_V_phi.md`](./parf/On_Gumbel_softmax_sparsity_applied_to_V_phi.md) — Stage 1.5 design; the score-head and Gumbel-routing infrastructure reused here.
