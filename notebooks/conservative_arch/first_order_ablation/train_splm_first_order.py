"""
Training loop for the SPLM-1 first-order ablation.

Identical to ../ln_damping_sweep/train_splm_em_ln.py except it instantiates
ScalarPotentialLMFirstOrder (no velocity buffer, no damping term, just
gradient flow on V_theta) instead of ScalarPotentialLMSARFMassLN.

The script intentionally accepts (and ignores) --fixed-gamma so that the
sweep harness can call this script identically to the E5 trainer; the
first-order integrator does not consult gamma. The fixed_gamma argument is
echoed to the summary file for provenance only.
...
from model_first_order import (  # noqa: E402
    ScalarPotentialLMFirstOrder,
    SPLMFirstOrderConfig,
)
...
def build_config(
...
) -> tuple[SPLMFirstOrderConfig, dict]:
    base_kw = dict(
        vocab_size=50257,
        mass_mode="logfreq",
    ...
    if mode == "smoke":
        model_cfg = SPLMFirstOrderConfig(
...
@torch.no_grad()
def evaluate(model: ScalarPotentialLMFirstOrder, ids: np.ndarray,
...
def main():
    ...
    ap.add_argument(
        "--fixed-gamma", dest="fixed_gamma", type=float, default=0.0,
        help="Echoed to checkpoint metadata for provenance; the first-order "
             "integrator does not consult it.",
    )
    ap.add_argument(
    ...
    print(f"[train-fo] device={device}  mode={args.mode}  "
    ...
    print(f"[train-fo] tokens: train={len(train_ids):,}  val={len(val_ids):,}")
    ...
    model = ScalarPotentialLMFirstOrder(model_cfg).to(device)
    n_params = model.num_params()
    print(f"[train-fo] params: {n_params:,}   d={model_cfg.d}  "
    ...
    print(f"[train-fo] init mass: mean={init_mass['mean']:.3f}  "
    ...
    tag = f"splm_first_order_{args.mode}"
    if args.tag_suffix:
    ...
    for step in range(train_cfg["steps"]):
        ...
        if (step + 1) % train_cfg["log_interval"] == 0:
            ...
            print(f"[train-fo] step {step+1:5d}/{train_cfg['steps']}   "
        ...
        if (step + 1) % train_cfg["eval_interval"] == 0:
            ...
            print(f"[train-fo] step {step+1:5d}   val_loss={val_loss:.4f}  "
    ...
    print(f"\n[train-fo] DONE  val_loss={final_val:.4f}  "
          f"val_ppl={final_ppl:.2f}  (first-order, no gamma)")
    ...
    torch.save({
        ...
        "variant": "splm_first_order",
        "tag": tag,
        "seed": args.seed,
    }, ckpt_path)
    print(f"[train-fo] checkpoint saved to {ckpt_path}")
    ...
    if steps_v:
        ax.plot(steps_v, [math.exp(v) for v in val_vs],
                label="val ppl", color="firebrick")
    ax.set_xlabel("step")
    ax.set_ylabel("perplexity")
    ax.set_title(f"SPLM-1 (first-order) — {args.mode} — seed={args.seed}")
    ax.legend()
    ...
    with summary_path.open("w") as f:
        ...
        f.write(f"- Model: ScalarPotentialLMFirstOrder (SPLM-1)\n")
        f.write(f"- mode: {args.mode}\n")
        f.write(f"- variant: splm_first_order (no v-buffer, no gamma)\n")
    ...
    print(f"[train-fo] summary written to {summary_path}")
