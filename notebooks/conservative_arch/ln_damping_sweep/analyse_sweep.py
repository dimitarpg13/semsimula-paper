"""E5 LN damping sweep — aggregate per-cell results, plot, emit RESULTS.md.

Identical logic to damping_sweep/analyse_sweep.py but reads em_ln summary files
(prefix: splm_em_ln_shakespeare_<tag>_summary.md) and titles all plots/reports
with "em_ln" labels.
...
def _read_cell(cell_dir: Path, tag: str) -> CellRow | None:
    if not cell_dir.exists():
        return None
    summary_md = next(
        cell_dir.glob(f"splm_em_ln_shakespeare_{tag}_summary.md"),
        None,
    )
    ...
    return CellRow(
        ...
        drift_slope_per_layer=float(drift.get("drift_slope_per_layer", float("nan"))),
...
def _plot_axis_curve(rows: list[CellRow], y_attr: str, ylabel: str,
    ...
    ax.plot(xs, ys, marker="o", linewidth=2, markersize=8, color="darkorange")
    if baseline_value is not None:
        ax.axhline(baseline_value, linestyle="--", color="grey", alpha=0.7,
...
def _format_results_md(rows: list[CellRow], out_path: Path) -> None:
    lines: list[str] = []
    lines.append("# RESULTS — E5 SPLM em_ln (LN-after-step) damping sweep\n")
    lines.append("> Companion to E4 (plain Euler sweep): "
                 "[`../damping_sweep/results/RESULTS.md`](../damping_sweep/results/RESULTS.md)\n")
    lines.append("")
    lines.append("## Headline grid\n")
    lines.append(
        "| tag | gamma | val ppl | drift slope / layer | "
        "bandwidth | rho_12 | p_12 | Markov decision | "
        "a_par<0 | |a_par|/|a_perp| | "
        "perm z |"
    )
    lines.append(
        "|---|---:|---:|---:|---:|---:|---:|---|---:|---:|---:|"
...
def main():
    ...
    print(f"[E5-analyse] wrote {summary_path}")

    _plot_axis_curve(
        rows, "val_ppl", "val perplexity",
        args.fig_dir / "ppl_vs_gamma.png",
        title="SPLM em_ln val PPL vs damping coefficient",
    )
    _plot_axis_curve(
        rows, "drift_slope_per_layer", "drift slope per layer (units of |H_0|)",
        args.fig_dir / "drift_slope_vs_gamma.png",
        title="E5 energy-drift slope vs damping (em_ln)",
    )
    _plot_axis_curve(
        rows, "rho_12", "rho_12 = R1 / R2",
        args.fig_dir / "markov_rho12_vs_gamma.png",
        title="Markov-order regression: rho_12 vs damping (em_ln)",
        baseline_value=1.0,
        baseline_label="rho_12=1 (no preference)",
    )
    _plot_axis_curve(
        rows, "frac_a_par_negative",
        "Pr[a_par < 0]",
        args.fig_dir / "apar_negative_vs_gamma.png",
        title="§14 trajectory shape: a_par<0 rate vs damping (em_ln)",
        baseline_value=0.979,
        baseline_label="natural GPT-2 (97.9 %)",
    )
    print(f"[E5-analyse] wrote 4 figures into {args.fig_dir}")

    rmd = args.results_root / "RESULTS.md"
    _format_results_md(rows, rmd)
    print(f"[E5-analyse] wrote {rmd}")
    return 0
