"""
Trajectory extraction for SARF-faithful SPLM.

Mirrors `../trajectory_extraction.py` but loads a ScalarPotentialLMSARF
checkpoint.  The output Trajectory objects have the same layout
(hs: (L+1, T, d), per-layer cumulative-mean xi is recomputed from hs at
analysis time if needed), so the existing `../shared_potential_fit.py`
and `../token_direction_fit.py` diagnostics work drop-in.

Usage:
  python3 trajectory_extraction_sarf.py \
...
def main():
    ...
    print(f"[extract-sarf] device={device}  ckpt={args.ckpt}")

    ck = torch.load(args.ckpt, map_location=device, weights_only=False)
    model_cfg = SPLMSARFConfig(**ck["model_cfg"])
    model = ScalarPotentialLMSARF(model_cfg).to(device)
    model.load_state_dict(ck["model_state_dict"])
    model.eval()
    print(f"[extract-sarf] loaded model with d={model_cfg.d}  L={model_cfg.L}  "
          f"max_len={model_cfg.max_len}  variant={ck.get('variant', 'sarf')}")
    ...
    print(f"[extract-sarf] extracted {len(trajs)} trajectories")
    ...
    print(f"[extract-sarf] saved -> {out_path}")
