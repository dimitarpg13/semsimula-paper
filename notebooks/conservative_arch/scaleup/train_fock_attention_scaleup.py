"""
Training loop for the **Fock Attention** scale-up experiment.

Implements the literal §5.1 Feynman diagram (direct token-to-token exchange
force) on top of the Multi-Xi PARFLM backbone.  This is the λ=0 instantaneous
exchange limit — no registers, no persistence — used to isolate the
contribution of directed non-conservative exchange.

Hard-fork of ``train_fock_multixi_scaleup.py`` with the model class swapped:

    FockMultiXiPARFLM       (K-EMA ξ + sparse PARF + Fock registers)
    →
    FockAttentionPARFLM     (K-EMA ξ + sparse PARF + direct exchange force)

All multi-xi PARF hyperparameters (V_φ, score head, Gumbel-softmax, P8 patches,
K-EMA ξ) are inherited unchanged.  Exchange-specific flags: --exchange-n-heads,
--exchange-d-k, --exchange-scale-init.
...
def build_config(
    ...
    *,
...
) -> tuple[FockAttentionConfig, dict, str]:
    ...
    cfg = FockAttentionConfig(**base_kw)
    ...
    head_tag = f"_h{exchange_n_heads}" if exchange_n_heads > 1 else ""
    tag = (f"fock_attention_multixi_K{xi_channels}_{v_phi_kind}_vphi{phi_hidden}"
           f"{gc_tag}{lc_tag}{gv_tag}{fg_tag}{p8_tag}"
           f"_sparse_k{sparse_top_k}_dk{exchange_d_k}{head_tag}_{mode}")
...
def main():
    ap = argparse.ArgumentParser(
        description="Fock Attention (direct exchange force) scale-up trainer"
    )
    ...
    ap.add_argument("--exchange-n-heads", dest="exchange_n_heads", type=int,
                    default=1,
                    help="Number of attention heads in the exchange force.")
    ap.add_argument("--exchange-d-k", dest="exchange_d_k", type=int,
                    default=64,
                    help="Per-head key/query dimension for exchange force.")
    ap.add_argument("--exchange-scale-init", dest="exchange_scale_init",
                    type=float, default=0.0,
                    help="Initial exchange gate value (0 = force off at init).")
    ap.add_argument("--exchange-grad-clip", dest="exchange_grad_clip",
                    type=float, default=None,
                    help="Separate (tighter) grad clip for exchange params.")
    ap.add_argument("--disable-exchange", dest="disable_exchange",
                    action="store_true", default=False,
                    help="Fix exchange scale at 0 (control arm: no force).")
    ...
    if torch.cuda.is_available():
        ...
        print("[fock-attention] TF32 disabled for autograd.grad stability")
    ...
    print(f"[fock-attention] tokens: train={len(train_ids):,}  "
          f"val={len(val_ids):,}")
    ...
    cfg, train_cfg, base_tag = build_config(
        ...
        exchange_n_heads=args.exchange_n_heads,
        exchange_d_k=args.exchange_d_k,
        exchange_scale_init=args.exchange_scale_init,
    )
    ...
    model = FockAttentionPARFLM(cfg).to(device)

    # Control arm: freeze exchange scale at 0 so force is always off
    if args.disable_exchange:
        model.exchange_scale.requires_grad_(False)
        model.exchange_scale.fill_(0.0)
        print("[fock-attention] exchange force DISABLED (control arm)")
    ...
    n_exchange = model.get_exchange_overhead()
    alpha_str = ", ".join(f"{a:.3f}" for a in model.xi_alpha_values())

    print(f"[fock-attention] device={device}  tag={tag}")
    print(f"[fock-attention] params: {n_params:,}  V_theta={n_v_theta:,}  "
          f"V_phi={n_v_phi:,}  score_head={n_score:,}  "
          f"xi_module={n_xi:,}  exchange_oh={n_exchange:,}")
    print(f"[fock-attention] arch: d={cfg.d}  L={cfg.L}  "
          f"v_hidden={cfg.v_hidden}  V_phi={cfg.v_phi_kind!r}  "
          f"top_k={cfg.top_k}  fixed_gamma={cfg.fixed_gamma}")
    print(f"[fock-attention] xi: K={cfg.xi_channels}  "
          f"learnable={cfg.xi_learnable}  "
          f"mode={cfg.xi_alpha_init_mode}  \u03b1=[{alpha_str}]")
    print(f"[fock-attention] exchange: n_heads={cfg.exchange_n_heads}  "
          f"d_k={cfg.exchange_d_k}  scale_init={cfg.exchange_scale_init}")
    print(f"[fock-attention] schedule: {args.mode}  "
          f"steps={train_cfg['steps']}  batch={train_cfg['batch_size']}  "
          f"block={train_cfg['block_size']}")

    exchange_grad_clip = args.exchange_grad_clip
    _exchange_param_list: list[nn.Parameter] = []
    for name, param in model.named_parameters():
        if any(k in name for k in (
            "exchange_force", "exchange_scale",
        )):
            _exchange_param_list.append(param)
    _vphi_param_list = list(model.V_phi.parameters())
    if exchange_grad_clip is not None:
        print(f"[fock-attention] exchange-grad-clip: {exchange_grad_clip} "
              f"({len(_exchange_param_list)} params)")

    if not args.skip_causal_check:
        print("[fock-attention] running causal-violation probe ...")
        assert_causal(model, vocab_size=cfg.vocab_size, T=32, t_pert=20)
        print("[fock-attention] causal probe passed.")
    else:
        print("[fock-attention] causal probe SKIPPED "
              "(--skip-causal-check).")
    ...
    if grad_accum > 1:
        print(f"[fock-attention] grad-accum: {grad_accum} micro-batches "
    ...
    for step in range(train_cfg["steps"]):
        ...
        _pre_clip_ex_gn = nn.utils.clip_grad_norm_(
            [p for p in _exchange_param_list if p.grad is not None],
            float("inf"),
        ).item() if _exchange_param_list else 0.0
        ...
        grad_norm = nn.utils.clip_grad_norm_(model.parameters(),
                                             train_cfg["grad_clip"])
        if exchange_grad_clip is not None and exchange_grad_clip < train_cfg["grad_clip"]:
            ex_params = [p for p in _exchange_param_list if p.grad is not None]
            if ex_params:
                nn.utils.clip_grad_norm_(ex_params, exchange_grad_clip)
        optim.step()
        ...
        if (step + 1) % train_cfg["log_interval"] == 0:
            ...
            ex_diag = model.exchange_diagnostics()
            ex_str = f"   ex_s={ex_diag['exchange_scale']:.3f}"

            print(
                f"[fock-attention] step {step+1:5d}/{train_cfg['steps']}   "
                f"train {avg:.4f}   lr {lr_now:.2e}   "
                f"grad {grad_norm:.2f}  "
                f"(exch={_pre_clip_ex_gn:.2f} vphi={_pre_clip_vphi_gn:.2f})"
            ...
            log_entry = {
                ...
                "grad_norm_exchange": _pre_clip_ex_gn,
        ...
        if ((step + 1) % train_cfg["eval_interval"] == 0
            ...
            print(f"[fock-attention] >>> eval @ {step+1}: "
    ...
    print(
        f"\n[fock-attention] DONE  val_loss={final_val:.4f}  "
    ...
    torch.save({
        ...
        "variant": "fock_attention_multixi_sparse",
        "experiment": "fock_attention_scaleup",
        "tag": tag,
        "seed": args.seed,
        "n_params": n_params,
        ...
        "n_exchange_overhead_params": n_exchange,
        "exchange_n_heads": cfg.exchange_n_heads,
        "exchange_d_k": cfg.exchange_d_k,
        "elapsed_sec": total_elapsed,
        "final_gumbel_tau": model.gumbel_tau,
        "top_k": cfg.top_k,
    }, ckpt_path)
    print(f"[fock-attention] checkpoint -> {ckpt_path}")
    ...
    ax.set_title(
        f"Fock Attention K={cfg.xi_channels} "
        f"h={cfg.exchange_n_heads} dk={cfg.exchange_d_k} "
    ...
    with summary_path.open("w") as f:
        ...
        f.write("- experiment: Fock Attention (direct exchange force) scale-up\n")
        f.write(f"- model: FockAttentionPARFLM "
                f"(K-EMA \u03be + sparse PARF + \u00a75.1 exchange)\n")
        ...
        f.write(f"- exchange: n_heads={cfg.exchange_n_heads}  "
                f"d_k={cfg.exchange_d_k}  "
                f"scale_init={cfg.exchange_scale_init}\n")
        ...
        f.write(f"Final exchange scale: "
                f"{torch.tanh(model.exchange_scale).item():.4f}\n")
    print(f"[fock-attention] summary -> {summary_path}")
