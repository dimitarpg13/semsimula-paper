"""
Training loop for the **E9 SPLM scale-up de-risking experiment** (SPLM arm).

Pre-registered protocol:
  docs/SPLM_scaleup_pre-registered_protocol.md
Pre-registration commit: 17a3795

This trainer is a hard-fork of `ln_damping_sweep/train_splm_em_ln.py` adapted
for the scale-up configuration locked in the protocol:
  - corpus      : TinyStories (~5 M GPT-2 BPE tokens)
  - max_len     : 1024
  - block_size  : 512
  - d / L / v_h : 256 / 8 / 1024  (~15.75 M params)
  - mass        : logfreq, alpha-init 0.1, surprisal computed on TinyStories
  - damping     : fixed gamma = 0.30 (E5 winner)
...
-----
  ...
  --mode scaleup       : full E9 pre-registered run (8000 steps, 400 warmup, 400 eval).
  --mode pilot         : E10 γ-transfer Stage 1 (4000 steps, 200 warmup, 200 eval).
                         Pre-registration: docs/Gamma_transfer_pre-registered_protocol.md (commit 75cad01).
  --mode confirmation  : E10 γ-transfer Stages 2+3 (identical to `scaleup`, but tagged
                         as a γ-transfer confirmation run for bookkeeping).
"""
...
def build_config(
    ...
    if mode == "smoke":
        # Same config as scaleup but only 300 steps + 256-token blocks so we
        # verify the pipeline (data load, MPS, gradient flow) at scale-up
        # parameter count without burning the whole night.
    ...
    elif mode == "scaleup" or mode == "confirmation":
        model_cfg = SPLMSARFMassLNConfig(
            d=256, max_len=1024, v_hidden=1024, v_depth=3, L=8,
            init_m=1.0, init_gamma=1.0,
            **base_kw,
        ...
        # E10 γ-transfer Stage 1: half-schedule pilot used only for *ranking*
        # γ values, not for headline PPL claims. Schedule is the E9 'scaleup'
        # schedule scaled by 0.5 in steps/warmup/eval_interval; everything else
        # (model size, batch, block, lr peak, optimiser) is identical.
        model_cfg = SPLMSARFMassLNConfig(
            d=256, max_len=1024, v_hidden=1024, v_depth=3, L=8,
            init_m=1.0, init_gamma=1.0,
            **base_kw,
...
def main():
    ...
    ap.add_argument(
        "--mode",
        choices=["smoke", "scaleup", "pilot", "confirmation"],
        default="smoke",
    )
    ...
    print(f"[scaleup-splm] tokens: train={len(train_ids):,}  val={len(val_ids):,}")
    ...
    print(f"[scaleup-splm] params: {n_params:,}   d={model_cfg.d}  "
    ...
    print(f"[scaleup-splm] init mass: mean={init_mass['mean']:.3f}  "
    ...
    tag = f"splm_em_ln_scaleup_{args.mode}"
    if args.tag_suffix:
    ...
    for step in range(train_cfg["steps"]):
        ...
        if (step + 1) % train_cfg["log_interval"] == 0:
            ...
            print(f"[scaleup-splm] step {step+1:5d}/{train_cfg['steps']}   "
        ...
        if (step + 1) % train_cfg["eval_interval"] == 0:
            ...
            print(f"[scaleup-splm] step {step+1:5d}   val_loss={val_loss:.4f}  "
    ...
    print(f"\n[scaleup-splm] DONE  val_loss={final_val:.4f}  "
    ...
    print(f"[scaleup-splm] checkpoint saved to {ckpt_path}")
    ...
    ax.set_title(f"SPLM em_ln scale-up — {args.mode} ({gamma_str}) — seed={args.seed}")
    ax.legend()
    ...
    with summary_path.open("w") as f:
        ...
        f.write(f"- experiment: E9 scale-up (SPLM arm)\n")
    ...
    print(f"[scaleup-splm] summary written to {summary_path}")
