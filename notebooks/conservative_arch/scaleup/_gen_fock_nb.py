#!/usr/bin/env python3
"""Generate colab_fock_multixi_structured_vtheta.ipynb"""
import json, pathlib
...
# ── Cell 1 ─────────────────────────────────────────────────────────────
md("## 0. Environment setup + cell selector")
...
code(
    ...
    "    ax.set_title(f'Structured V_\\u03b8 on Multi-Xi Fock-PARFLM v2.1 \\u2014 {CELL}')\n"
    "    ax.legend()\n"
...
# ── Cell 17 ────────────────────────────────────────────────────────────
md("## 8. V_\\u03b8 landscape diagnostics")
...
code(
    ...
    "ax.set_xlabel('V_\\u03b8(\\u03be, h)')\n"
    "ax.set_ylabel('count')\n"
    "ax.set_title(f'{CELL} V_\\u03b8 distribution ({recipe[\"desc\"]})')\n"
    "ax.legend()\n"
    "plt.tight_layout()\n"
...
# ── Write notebook ─────────────────────────────────────────────────────
out = pathlib.Path(
    "/Users/dimitargueorguiev/git/ml/semsimula-paper/notebooks/"
    "conservative_arch/scaleup/colab_fock_multixi_structured_vtheta.ipynb"
)
with open(out, "w") as f:
    json.dump(NB, f, indent=1, ensure_ascii=False)
print(f"Wrote {len(NB['cells'])} cells -> {out}")
